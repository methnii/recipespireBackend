...
export class IngredientFormComponent implements OnInit {
  ...
  loadIngredient(id: number) {
    ...
    this.ingredientService.getIngredientById(id).subscribe({
      ...
      error: (error) => {
        ...
        this.toastService.showError('Error', 'Failed to load ingredient');
        this.loading = false;
  ...
  onSubmit() {
    if (this.ingredientForm.valid) {
      ...
      operation.subscribe({
        next: () => {
          const message = this.isEditMode ? 'Ingredient updated successfully' : 'Ingredient created successfully';
          this.toastService.showSuccess('Success', message);
          this.router.navigate(['/ingredients']);
        },
        error: (error) => {
          console.error('Error saving ingredient:', error);
          this.toastService.showError('Error', 'Failed to save ingredient');
          this.loading = false;
        }
      });
    }
