...
@Component({
  ...
  template: `
    <div *ngIf="toast" 
         class="fixed top-4 right-4 z-50 max-w-sm w-full bg-white shadow-lg rounded-lg pointer-events-auto ring-1 ring-black ring-opacity-5 overflow-hidden transform transition-all duration-300"
  ...
  ngOnChanges() {
    if (this.toast) {
      ...
      if (this.toast.duration !== 0) {
        setTimeout(() => this.close(), this.toast.duration || 5000);
      }
    }
  }
